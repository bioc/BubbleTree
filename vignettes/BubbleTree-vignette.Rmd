...
## BubbleTree plot for lung cancer WGS samples using adjusted and non-adjusted CNV data.

```{r}
load(system.file("data", "allCall.lst.RData", package="BubbleTree"))
btreeplotter <- new("BTreePlotter", max.ploidy=5, max.size=10)
sample <- allCall.lst[["lung.wgs"]]
rbd1 <- sample@rbd
rbd2 <- sample@rbd.adj
arrows <- trackBTree(btreeplotter, rbd1, rbd2, min.srcSize=0.01, min.trtSize=0.01)
...
## BubbleTree plot for lung cancer WGS samples using only non-adjusted CNV data.
...
<img src="figures/HCC11_comparison.png"/ style="width: 100%; height: auto; border: 1px solid;"/>
</div>
